...
fluidPage(
  ...
  div(class='jumbotron'
      ...
      color: #008cba;
      "
      ,actionButton("goButton001", "About Emofiel", styleclass = 'primary'),
  div(class='jumbotron'
      , style="
      text-align:center;
      display: inline-block;
      ...
      "
      , titlePanel(h2(
        "EMOFIEL: EMOtion mapping of FIctional rELationship"
        )
      )
  ),
  div(
...
,selectInput('x', label = 'Which story?' 
      ...
      span(style="
           ...
           , textInput.typeahead(
             id="user_char1input"
             ,placeholder="Type the first character name"
      ...
      span(style="
        ...
        "
           , textInput.typeahead(
             id="user_char2input"
             ,placeholder="Type the second character name"
             ,local=data.frame(name=c(list_of_character),info=c(list_of_character_desc))
             ,valueKey = "name"
             ,tokens=c(1:length(list_of_character))
             ,template = HTML("<p style='font-size: 16px;'><b>{{name}}</b></p> <p style='font-size: 13px;'>{{info}}</p>")
